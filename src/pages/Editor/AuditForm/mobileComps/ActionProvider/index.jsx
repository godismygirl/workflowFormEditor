...
const ActionProvider = ({
  ...
  const {
    ...
    hasError,
    endDragAction,
  } = useModel('useFormLayout');

  const { removeConfig } = useModel('useWorkflow');
  const { resetByFormLayoutChange } = useModel('useSaveProgress');
  ...
  const removeComp = (evt) => {
    ...
    Modal.confirm({
      title: '确定要删除吗',
      content: '将同步删除打印设置，审批流程，列表设计，扩展设置中的关联内容',
      okText: '确定',
      cancelText: '取消',
      okType: 'danger',
      onOk: () => {
        deleteAction();
        resetByFormLayoutChange(id, type === COMP_NAMES.EMPLOYEE_PICKER);
      },
    });
  };
