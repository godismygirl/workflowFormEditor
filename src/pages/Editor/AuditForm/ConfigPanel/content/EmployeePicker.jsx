import React, { useEffect } from 'react';
import { useModel } from 'umi';
import { Form, Input, Switch } from 'antd';

const EmployeePickerConfig = () => {
  const [form] = Form.useForm();
  const { activeItem, updateAttrs } = useModel('useFormLayout');
  const { updateConfig } = useModel('useWorkflow');

  const update = (changed, formData) => {
    updateAttrs(activeItem.id, { ...formData });
    if (changed.label) {
      //修改人员选择组件label的话，还要同步更新workflow里的审批人-表单内联系人配置
      updateConfig(activeItem.id, formData.label);
    }
  };

  useEffect(() => {
    if (activeItem.id) {
      form.setFieldsValue(activeItem?.attrs);
    }
  }, [activeItem.id]);

  return (
    <Form form={form} layout="vertical" onValuesChange={update}>
      <Form.Item label="标签名" name="label">
        <Input />
      </Form.Item>
      <Form.Item label="空白提示" name="placeholder">
        <Input />
      </Form.Item>
      <Form.Item label="是否多选" name="multiple" valuePropName="checked">
        <Switch checkedChildren="是" unCheckedChildren="否" />
      </Form.Item>
      <Form.Item label="是否必填" name="required" valuePropName="checked">
        <Switch checkedChildren="是" unCheckedChildren="否" />
      </Form.Item>
    </Form>
  );
};

export default React.memo(EmployeePickerConfig);
